...
function loadChannelsConfig(configPath) {

  return getChannelNames(configPath).then(function (channels) {

    return Promise.all(channels.map(function (channelName) {
      return Promise.all(conf_keys.map(function (key) {
        return config.loadJson(path.join(configPath, channelName, key + CONF_SUFFIX));
...
function getChannelNames(configPath) {
